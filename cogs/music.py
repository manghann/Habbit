...
class YTDLSource(discord.PCMVolumeTransformer):
    ...
    def parse_duration(duration: int):
        ...
        if hours > 0:
            duration.append('{} hrs'.format(hours))
        if minutes > 0:
            duration.append('{} mins'.format(minutes))
        if seconds > 0:
            duration.append('{} secs'.format(seconds))
    ...
    async def audio_player_task(self):
        while True:
            ...
            if not self.loop:
                ...
                # If no song will be added to the queue in time, the player will disconnect due to performance reasons.
                try:
                    async with timeout(480):  # 3 minutes
...
class Music(commands.Cog):
    ...
    async def cog_command_error(self, ctx: commands.Context, error: commands.CommandError):
        print("cog_command_error:" + str(error))


    @commands.command(name='join', invoke_without_subcommand=True) #  Joins a voice channel.
    ...
    @commands.command(name='summon')         # Summons the bot to a voice channel. If no channel was specified, it joins your channel.
    ...
    @commands.command(name='leave', aliases=['disconnect'])     # Clears the queue and leaves the voice channel.
    @commands.has_permissions(manage_guild=True)
    async def _leave(self, ctx: commands.Context):
        
        if not ctx.voice_state.voice:
            await ctx.send('⚠️ ' + ctx.message.author.mention + ', connect to a voice channel, please! (•ㅅ•)') 
    ...
    @commands.command(name='stop')        # Stops playing song and clears the queue.
    @commands.has_permissions(manage_guild=True)
    ...
    @commands.command(name='skip', aliases=['skipto'])        # The requester can automatically skip to a specific enqueued track.
    ...
    @commands.command(name='loop')        # Loops the currently playing song. Invoke this command again to unloop the song.
    ...
    async def _play(self, ctx: commands.Context, *, search: str):
        ...
        If there are songs in the queue, this will be queued until the other songs finished playing.
    ...
    async def ensure_voice_state(self, ctx: commands.Context):
        ...
        if ctx.voice_client:
            if ctx.voice_client.channel != ctx.author.voice.channel:
                raise commands.CommandError("Habbit's Music Buddy is already in a voice channel!")
